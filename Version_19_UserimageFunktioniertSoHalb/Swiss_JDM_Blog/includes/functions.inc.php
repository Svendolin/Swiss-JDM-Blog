...
// TODO: NEW SRUFF --- IMAGEUPLOAD-PART --- //
...
function createUser($conn, $name, $username, $car, $userimage, $state, $email, $pwd) {
  $sql = "INSERT INTO users (usersName, usersUid, usersCar, usersImage, usersState, usersEmail, usersPwd) VALUES (?, ?, ?, ?, ?, ?, ?);"; // Prepare Statement (CUDR CREATE Operator):
	...
	mysqli_stmt_bind_param($stmt,"sssssss", $name, $username, $car, $newImage, $state, $email, $hashedPwd); // (*) Changed variable $pwd to $hashedPwd here for extra safety
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);
	mysqli_close($conn);
