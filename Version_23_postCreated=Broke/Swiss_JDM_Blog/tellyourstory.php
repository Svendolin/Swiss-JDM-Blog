...
if (isset($_POST['post_title']) && isset($_POST['post_created']) && isset($_POST['post_author']) && isset($_POST['post_longtext'])) {
  ...
  // print_r($_POST);
  // echo '</pre>';
  $allowed_tags = array('a', 'img', 'strong', 'em', 'p', 'blockquote', 'div', 'span', 'ul', 'ol', 'li');
  ...
  $text = strip_tags(mysqli_real_escape_string($conn, $_POST['post_longtext']), $allowed_tags); // hier werden alle vom Editor angebotenen Tags vom entfernen ausgeschlossen...
  $ID = (int)$_POST['ID'];
...
<section id="storytellingform">
  ...
  <form action="" method="POST" class="write-a-post" enctype="multipart/form-data">
    ...
    <div class="input-field-story">
      ...
      <input type="file" name="post_image" id="fld_image" value="" required>
    </div>
    <?php if (!empty($image)) { ?>
        <img src="blogpost_images/<?php echo $image; ?>" style="" /><br><br>
      <?php } ?>
