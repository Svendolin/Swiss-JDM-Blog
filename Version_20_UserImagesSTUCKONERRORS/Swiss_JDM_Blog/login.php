...
if (isset($_POST['name']) && isset($_POST['uid']) && isset($_POST['car']) && isset($_POST['state']) && isset($_POST['email']) && isset($_POST['pwd']) && isset($_POST['pwdrepeat'])) {
  ...
  // FIXME: WE GET STUCK HERE!! -> When we comment this out, the image will be posted
  ...
  if (!$hasError) {
    ...
    if (empty($ID)) {
      $addImageField = empty($newImage) ? "" : ", users_image"; // Addition for image, only if image has been uploaded
      $addImageValue = empty($newImage) ? "" : ", ?"; // Addition for image, only if image has been uploaded

      $query = "INSERT INTO users (usersName, usersUid, usersCar, usersState, usersEmail, usersPwd {$addImageField})
         VALUES 
         (?, ?, ?, ?, ?, ? {$addImageValue})";
      ...
      if (empty($newImage)) {
        // Falls ohne Bild:
        mysqli_stmt_bind_param($res, "ssssss", $name, $username, $car, $state, $email, $hashedPwd); // send the data to the server
      } else {
        // Falls mit Bild:
        mysqli_stmt_bind_param($res, "sssssss", $name, $username, $car, $state, $email, $hashedPwd, $newImage); // send the data to the server
      }
