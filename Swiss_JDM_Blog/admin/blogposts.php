  ...
  <?php } ?>
    ...
    <div class="informationzone">
      <h2 class="info">
        ...
        <i class="fas fa-exclamation-triangle"></i> Timestamp does not reset during processing and remains at the very first release date. <br>
      ...
      <tbody>
        <?php if (isset($data) && count($data) > 0) { ?>
          <?php foreach ($data as $datapieces) { ?>
            <?php
            $ts = strtotime($datapieces['post_created']);
            $created = strftime("%d.%m.%Y - %H:%M:%S", $ts);
            ?>
            <tr>
              <td data-label="ID"><?php echo $datapieces['IDblogpost']; ?></td>
              <td data-label="Title"><?php echo stripslashes($datapieces['post_title']); ?></td>
              <td data-label="Release Date"><?php echo $created; ?></td>
